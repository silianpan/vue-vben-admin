...
export const createAntdModal = {
  install(app, originBaseOption) {
    app.config.globalProperties.$createAntdModal = function (options, argObj, argLocation) {
...
export const createViewModal = {
  install(app, originBaseOption) {
    app.config.globalProperties.$createViewModal = function (options, argObj, argLocation) {
      const { location, baseOption } = modifyOptions(originBaseOption, argObj, argLocation);
      const newOptions = locationMatcher.call(this, location, baseOption, options);
      const optionsWithGH = setGlobalHeader(baseOption, newOptions);
      return createModal(
...
export const createEleModal = {
  install(app, originBaseOption) {
    app.config.globalProperties.$createEleModal = function (options, argObj, argLocation) {
      const { location, baseOption } = modifyOptions(originBaseOption, argObj, argLocation);
      const newOptions = locationMatcher.call(this, location, baseOption, options);
      const optionsWithGH = setGlobalHeader(baseOption, newOptions);
      return createModal(
